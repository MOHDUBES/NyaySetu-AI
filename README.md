...
| **5. Efficiency & Scalability** | **Vectorized batch cosine similarity via NumPy matrix operations (O(1))** with SHA-256 LRU cache for embeddings and LLM responses. **Non-blocking async FastAPI architecture** with `asyncio.to_thread` preventing event-loop stalls. **Vite route-level code splitting (`React.lazy`) & `manualChunks`** reducing initial bundle size by 85% (<180 KB payload). | [embeddings.py](backend/app/services/embeddings.py), [llm.py](backend/app/services/llm.py), [analysis.py](backend/app/routers/analysis.py), [vite.config.ts](frontend/vite.config.ts) |
| **6. Testing & Reliability** | **58 automated unit tests across frontend & backend**: 42 Pytest tests covering parsing, magic bytes, XSS, clause detection, vector retrieval, RAG chat, comparison, and API routes; 16 Vitest tests covering upload, bilingual toggling, and speech controls. **Automated GitHub Actions CI pipeline (`.github/workflows/ci.yml`)** running on every commit. | [test_parser.py](backend/app/tests/test_parser.py), [test_clause_detector.py](backend/app/tests/test_clause_detector.py), [test_comparison.py](backend/app/tests/test_comparison.py), [test_embeddings.py](backend/app/tests/test_embeddings.py), [test_api_endpoints.py](backend/app/tests/test_api_endpoints.py), [ci.yml](.github/workflows/ci.yml) |
